...
Дополнение: [история, HYPE, DRZ и SMC](INDICATORS_DRZ_SMC.md). Глубина хранится в `vector.candles.v1`. Расчёт, загрузка MTF и отрисовка новых индикаторов разделены; параметры входят в существующие экземпляры workspace.
...
7. При открытии пары клиент загружает полную историю. Каждые 30 секунд видимая вкладка запрашивает две последние свечи через `/api/candles/latest` и сливает их с историей; при пропуске или смене провайдера загружает историю заново. Chart instance сохраняется, как и масштаб.
...
Кеш котировок: 30 секунд. Кеш свечей: 15 секунд. `refresh=1` обходит TTL; уже выполняющийся одинаковый запрос переиспользуется. Кеш максимум 64 ключа, журнал максимум 100 записей; API логов возвращает последние 60. Durable storage отсутствует. Дополнительные MTF-контексты активных индикаторов могут запрашивать полную историю при каждом обновлении основной свечи.
...
## Панели индикатора VMC

`PairWorkspace → useVmc → computeVmc → MarketChart → VmcRenderer`. Чистый вычислитель расположен в `src/indicators`, MTF-запросы — в hook, визуальное представление — в custom series. Каждому включённому экземпляру VMC соответствует отдельная native-панель в том же Lightweight Chart с общей временной шкалой. Отключение или удаление освобождает series, изменение порядка переносит панель. Независимые ценовые графики с разными symbol остаются будущим расширением. Подробности: [INDICATOR_VMC.md](INDICATOR_VMC.md).
