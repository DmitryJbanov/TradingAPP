...
Карта новых модулей истории, DRZ и SMC: [INDICATORS_DRZ_SMC.md](INDICATORS_DRZ_SMC.md).
...
### `candles(symbol, timeframe, config, force, count = 1000)`

Возвращает CandleResponse с выбранными 300–4000 свечами или доступной частью с warning. Частичная пригодная история сохраняет source=live; при отсутствии данных возвращается DEMO. Только известные инструменты и интервалы разрешены. UI использует 15m/1h/4h/1d; VMC/SMC могут загружать дополнительные интервалы.
